...
**A Transformer-free Vision-Language-Action model for real-time edge robotics.** Replacing $O(N^2)$ self-attention with Reaction-Diffusion PDEs.
...
As recently demonstrated by NVIDIA's own deployment of the Cosmos Reason 2B model on the Jetson Orin Nano, squeezing an $O(N^2)$ Transformer onto edge hardware requires crippling sacrifices:
...
$$u_{t+1}=\text{LayerNorm}\left(u_t+dt\cdot\left[\sum D_k\cdot\nabla^2(u_t)+R(u_t,\theta)+\alpha\cdot h_t\right]\right)$$

| Term | Role |
| --- | --- |
| $\sum D_k \cdot \nabla^2(u_t)$ | Multi-scale diffusion. Dilations [1, 4, 16]. $O(N)$. |
| $R(u_t, \theta)$ | Reaction MLP. Per-position nonlinear features. |
| $\alpha \cdot h_t$ | Memory pump. Global $O(1)$ accumulator - **no KV-cache, ever.** |
...
### 3. Memory - $O(T\cdot N)$ Video (The Core Claim)
...
See [RESULTS.md](RESULTS.md) for full benchmarks and [STRUCTURE.md](https://www.google.com/search?q=STRUCTURE.md) for project organization.
